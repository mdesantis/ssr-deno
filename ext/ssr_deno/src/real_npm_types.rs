...
///
/// Returns the checker, resolver, and package.json resolver so each consumer
/// (`DevModuleLoader`, `NodeExtInitServices`) constructs its own
/// `NodeResolver` from the same shared state. The `ByonmNpmResolver` is
/// `Clone`-cheap (it holds the shared `PackageJsonResolverRc` inside).
