...
9. Replace FFI stubs from step 2 with real logic. **Resolver dedup**: `DevModuleLoader::new()` (step 6) and `real_npm_types::build_dev_npm_resolver()` (step 5) both construct the same Byonm chain (`ByonmNpmResolver<Sys>` + `pkg_json_resolver` + `NodeResolutionSys`). Two sources of truth — drift risk. Either: (a) `DevModuleLoader::new` calls `build_dev_npm_resolver()` and reuses the pair; or (b) move the resolver wiring entirely into `dev_module_loader.rs` and drop `real_npm_types.rs`. **Also**: `build_dev_node_services()` in `dev_builder.rs` still uses `NopInNpmPackageChecker` / `NopNpmPackageFolderResolver` placeholders — wire the real Byonm pair here too, otherwise `require('react')` from JS hits Nop services and fails while `import 'react'` works through DevModuleLoader. Single resolver state, both consumers.
