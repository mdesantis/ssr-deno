...
Without the isolate pool, a single V8 isolate per Puma worker caps SSR throughput at `1 / renderToString_time`. The isolate pool (see `IsolatePool` in [`deno_runtime_wrapper/mod.rs`](../ext/ssr_deno/src/deno_runtime_wrapper/mod.rs)) addresses this within a single process, but a process pool remains relevant for use cases requiring failure isolation, multi-host scaling, or even higher throughput.
