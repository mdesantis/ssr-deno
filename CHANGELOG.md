...
- **Docker image no longer installs `clang-21`/`lld-21`** — vestigial from the pre-alpha.9 V8-from-source build; the actual linker is `mold` (via `RUSTFLAGS`), and `libclang-21-dev` (still installed, genuinely needed for `bindgen`) doesn't require the `clang` or `lld` binaries themselves.
