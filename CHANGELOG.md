...
### Fixed
- **`vite-react-streaming-ssr-app` sample broke under the newer V8** — its `renderToPipeableStream` usage destructured `pipe` from the call's return value, then referenced it from inside `onShellReady`. `onShellReady` can fire synchronously — inside the call itself, before the destructuring assignment runs — whenever the shell has no async dependency of its own, which is exactly this sample's shape (only the Suspense-wrapped child suspends). The old V8 (149.4.0) happened to never trigger that ordering; the new one (150.4.0) does, throwing `ReferenceError: Cannot access 'pipe' before initialization`. Fixed with the standard pattern for a callback that may fire before or after its initiating call returns: a `shellReady` flag checked from both sides.
