...
### Changed
- **CI now runs the full local gate** — added `cargo:test`, `cargo:test:ssr_deno_dev_mode`, and `cargo:test:ssr_deno` steps (Rust unit tests previously only ran locally via `bundle exec rake`); `coverage:check` now runs on `ruby: 3.3` too, in addition to `4.0` (previously the only version checked). `perf:check` stays local-only — the committed baselines were recorded on a specific machine class and GitHub runners are too noisy to compare against them meaningfully. `ruby: 3.4` still skips `coverage:check` — see Fixed.
- **`samples:build` rebuilds a sample when its sources changed, not just when `dist/` is missing** — it previously skipped any sample whose `dist/server/entry-server.js` already existed, so an edit to `src/`, `vite.config.*`, `deno.json`/`deno.lock`, or `package.json`/`package-lock.json` had no effect until you deleted `dist/` by hand. Now compares mtimes and rebuilds when any tracked source is newer than the bundle. New `samples:clean` task removes every sample's `dist/` to force a full rebuild.
...
### Fixed
- **Published gem no longer includes test-only Rust source** — `ext/ssr_deno/src/cjs_interop_repro_test.rs` (~340 lines, `#[cfg(test)]`-gated, never compiled into a release build) was shipped in every `gem build` regardless.
- **Docker image no longer installs `clang-21`/`lld-21`** — vestigial from the pre-alpha.9 V8-from-source build; the actual linker is `mold` (via `RUSTFLAGS`), and `libclang-21-dev` (still installed, genuinely needed for `bindgen`) doesn't require the `clang` or `lld` binaries themselves. Verified with a full `docker build`.
- **`coverage:check` skipped on `ruby: 3.4` in CI — upstream Ruby Coverage/GC race, not our code** — extending `coverage:check` to every matrix leg (see Changed) surfaced consistent failures on both `ruby: 3.4` legs, showing real code (e.g. `dev_mode_bundle.rb`) as uncovered despite its tests passing with 0 failures. Root-caused locally: any GC pass during a run can zero out already-recorded hit counts for lines that genuinely executed — a documented upstream limitation ([bugs.ruby-lang.org #22179](https://bugs.ruby-lang.org/issues/22179), [#22187](https://bugs.ruby-lang.org/issues/22187)), and Ruby 3.4 triggers more major GCs under concurrent workloads (this gem's isolate pool + dev-mode worker threads), making the race far more likely to fire than on 3.3/4.0. Confirmed via `GC.disable`: eliminates the flake (5/5 clean runs vs ~50% flaky with GC enabled), but disabling GC across the whole test process would defeat `test_deno_stability.rb`'s own purpose — its leak-detection assertions need GC to actually run. A single forced `GC.start` before result-collection only narrows the window (5/6 clean, not 6/6) since the corruption can happen from any GC pass mid-run, not just at the end. No safe application-level fix exists, so `ruby: 3.4` skips this one check; `3.3` and `4.0` (both 100% stable across 10+ local runs) keep it.
- **`AGENTS.md`, `Rakefile`, and `rakelib/test.rake` resynced with the actual suite/task list** — `AGENTS.md`'s "Test architecture" section listed only `test:main`/`test:node_builtins` (9 suites exist) and told the stale-audit checklist to check `lib/ssr/deno/bundle.rb` for `:nocov:` directives, of which zero remain anywhere in `lib/`. Replaced the suite table with a pointer to `rakelib/test.rake` instead of re-enumerating it (per this file's own "counts go stale" rule). `Rakefile`'s `rakelib/` file comment omitted `rbs.rake` entirely and `test.rake`'s own header comment omitted `test:ractor`/`test:rails`/`test:perf`.
- **README corrected: libclang rationale, prerequisites, and per-sample run commands** — libclang was attributed to "SQLite bindings"; it's actually needed because `rb-sys-build` runs `bindgen` to generate per-Ruby-ABI C API bindings (which can't be pre-shipped), and `libnghttp2`/`libsqlite3-sys` in the Deno dependency tree also run `bindgen`. Prerequisites were missing Node.js/npm, required by the `node-ssr-app` sample. The samples table said "run any with `deno task build && deno task serve`", which is wrong for 5 of 16 samples — added a **Run** column with the actual command per sample, and marked `vite-hmr-ssr-app` as a test fixture rather than a port-3115 app. The "Run tests" section's default-chain description was missing half the steps (`cargo:clippy`, `cargo:coverage`, `cargo:fmt`, `coverage:check`, `perf:check`, `rubocop:rails`).
- **`docs/architecture.md` and `docs/compatibility.md` corrected against the actual code** — `setup_require` no longer described as a 10ms microtask-spin (superseded by the alpha.9 `run_event_loop` fix); `Watchdog` and the `require()` rejection are both attributed to `render.rs`/`DenoNodeRequireLoader`, when they live in `watchdog.rs`/`SSRDenoNodeRequireLoader`; `Bundle`'s registry described as a `@@registry` built via a nonexistent `Bundle.bootstrap`, when it's a class-instance `@registry` populated by `create_bundles!`; the Ruby components table, source-file tree, and samples tree were missing `render_error.rb`, `rails/log_subscriber.rb`, `ractor_pool/worker.rb`, `rails/generators/`, `cjs_interop_repro_test.rs`, and the `vite-hmr-ssr-app` sample; the Testing table listed 5 of 9 suites, contradicting the (correct) suite list in the same file's source-tree comment. `CHANGELOG.md`'s own link to `plans/ssr-source-dev-mode.md` (moved to `plans/archived/` in alpha.9) is fixed too.
- **Dev-mode CJS interop broke when `project_root` resolved through a symlinked path component** — `DevModeModuleLoader` computed `node_modules_dir` from the raw, un-canonicalized `project_root`, while Node module resolution returns canonicalized paths. On macOS, where `/tmp` and `/var` are themselves symlinks to `/private/tmp` and `/private/var`, every `path.starts_with(&self.node_modules_dir)` check silently failed for a project rooted under either — misclassifying every CJS file in `node_modules` as non-CJS, so it fell through to being loaded as raw ESM with none of the exports the importer expected, surfacing as `SyntaxError: The requested module '<pkg>' does not provide an export named '<name>'`. Any `project_root` reached through a symlinked mount would trigger this identically. `node_modules_dir` is now canonicalized at construction, falling back to the raw path if `node_modules` doesn't exist yet. Found via `ext/ssr_deno/src/cjs_interop_repro_test.rs`, which this release makes part of the regular Rust test run for the first time (see Changed).
...
- **Ruby dev gems bumped** — `rubocop` 1.87.0→1.88.1, `rubocop-rails` 2.35.4→2.35.5, `rbs` 4.0.2→4.0.3, plus transitive gems (`nokogiri`, `i18n`, `mail`, `globalid`, `rdoc`, et al.) — all within existing `Gemfile` constraints, no version range changes needed.
...
- **`SSR::Deno::RenderError < SSR::Deno::Error` inheritance** — the class now explicitly inherits from `SSR::Deno::Error` (was implicit).
...
### Documentation
- **Experimental features marked and documented** — `SSR::Deno::RactorPool` and `SSR::Deno::DevModeBundle` are now explicitly flagged experimental in their class docstrings and in the README. Dedicated guides at `docs/ractor-pool.md` and `docs/dev-mode.md` cover requirements, usage, limitations, and feedback channels. Behaviour and interfaces of both features may change without a deprecation cycle.

### Fixed
- **Dev-mode CJS→ESM interop** — full MUI/emotion/React dependency graphs now load and render correctly in dev mode (no build step). The synthetic require() shim for `node_modules/*.js` files statically discovers CJS export names, ESM `.js` files are detected and loaded natively, subpackage resolution has a manual fallback, and a CJS warmup cache pre-loads all CJS modules via `execute_script` before `evaluate_module` to sidestep an upstream V8 re-entrancy bug. (`plans/archived/dev-mode-cjs-interop-bug.md`)
...
### Added
- **Dev-mode auto-reload** — `DevModeBundle` now supports `auto_reload = true`: checks source file mtimes before each render and respawns the Deno worker (fresh V8 isolate) on any change. Disabled by default. (`plans/archived/ssr-source-dev-mode.md` step 11)
