...
### Added
- V8 OOM protection: `near_heap_limit_callback` + `terminate_execution` prevents fatal process crash when a user SSR component exceeds `max_heap_size_mb`. V8 OOM now raises `SSR::Deno::RenderError` instead of `SIGTRAP`.
- Stability tests: leak detection (heap growth < 3x over 100 renders), large payload, edge-case data, rapid reload, OOM produces `RenderError`.
