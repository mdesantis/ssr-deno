...
### Changed
- **CI now runs the full local gate** — added `cargo:test`, `cargo:test:ssr_deno_dev_mode`, and `cargo:test:ssr_deno` steps (Rust unit tests previously only ran locally via `bundle exec rake`); `coverage:check` now runs on every matrix leg instead of only `ruby: 4.0`, so the 3.3 and 3.4 jobs verify coverage too. `perf:check` stays local-only — the committed baselines were recorded on a specific machine class and GitHub runners are too noisy to compare against them meaningfully.
