...
- **Use `rtk` command wrapper for all CLI operations.** `rtk` (https://github.com/rtk-ai/rtk) compresses command output to save tokens. Use `rtk git` instead of raw `git`, `rtk ls` instead of `ls`, `rtk read` instead of `cat`/`head`, `rtk test` for test output, etc. Always prefer `rtk <subcommand>` over the native tool when the equivalent exists.
